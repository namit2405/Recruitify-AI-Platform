...
class GoogleAuthInitView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response(
                {'error': 'Google OAuth not configured on this server. Please add credentials.json from Google Cloud Console.'},
...
class GoogleAuthCallbackView(APIView):
    ...
    def get(self, request):
        ...
        if error:
            ...
            return redirect(f"{settings.FRONTEND_URL}/settings?google_error={error}")
        
        if not code:
            from django.shortcuts import redirect
            return redirect(f"{settings.FRONTEND_URL}/settings?google_error=no_code")
        ...
        if not user_id:
            from django.shortcuts import redirect
            return redirect(f"{settings.FRONTEND_URL}/settings?google_error=no_user_id")
        ...
        try:
            ...
            from django.shortcuts import redirect
            return redirect(f"{settings.FRONTEND_URL}/settings?google_connected=true")
            
        except Exception as e:
            from django.shortcuts import redirect
            import traceback
            traceback.print_exc()
            return redirect(f"{settings.FRONTEND_URL}/settings?google_error={str(e)}")
...
class CheckGoogleConnectionView(APIView):
    ...
    def get(self, request):
        ...
        if request.user.user_type != 'organization':
            return Response({'connected': False, 'server_configured': False})
        ...
        server_configured = os.path.exists(credentials_file)
        ...
        try:
            ...
            return Response({'connected': False, 'server_configured': server_configured})
