...
import { fetchApi, fixMediaUrl, getToken } from '@/lib/api';
    ...
    queryFn: async () => {
      try {
        ...
        return {
          ...
          profile_picture_url: fixMediaUrl(data.profile_picture_url),
          cover_photo_url: fixMediaUrl(data.cover_photo_url),
        };
      } catch (error) {
    ...
    queryFn: async () => {
      try {
        ...
        return {
          ...
          resume_url: fixMediaUrl(data.resume_url),
          profile_picture_url: fixMediaUrl(data.profile_picture_url),
          cover_photo_url: fixMediaUrl(data.cover_photo_url),
          website_url: data.website_url,
          github_url: data.github_url,
...
export function useBulkDeleteNotifications() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (ids) => fetchApi('/notifications/bulk-delete/', {
      method: 'DELETE',
...
export function useCheckGoogleConnection() {
  ...
  return useQuery({
    ...
    queryFn: () => fetchApi('/video/google/check/'),
    enabled: !!token,
    staleTime: 30000,
  });
}
...
export function useDisconnectGoogle() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: () => fetchApi('/video/google/disconnect/', { method: 'POST' }),
    onSuccess: () => {
...
export function useInitiateGoogleAuth() {
  return useMutation({
    mutationFn: () => fetchApi('/video/google/auth/'),
  });
}
